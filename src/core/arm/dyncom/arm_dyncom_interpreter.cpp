...
#define BORKED3DS_IGNORE_EXIT(x)
...
get_addr_fp_t GetAddressingOpLoadStoreT(unsigned int inst) {
    ...
    // So it's sufficient for borked3ds to not implement this.
    return nullptr;
}
...
static int InterpreterTranslateBlock(ARMul_State* cpu, std::size_t& bb_start, u32 addr) {
    BORKED3DS_PROFILE("DynCom", "Decode");
...
unsigned InterpreterMainLoop(ARMul_State* cpu) {
    BORKED3DS_PROFILE("DynCom", "Execute");
...
BXJ_INST: {
    ...
    //
    // This is sufficient for borked3ds, as the CPU for the 3DS does not implement Jazelle.
