...
Result TranslateCommandBuffer(Kernel::KernelSystem& kernel, Memory::MemorySystem& memory,
    ...
    while (i < command_size) {
        ...
        case IPC::DescriptorType::MappedBuffer: {
            ...
            VAddr page_start = Common::AlignDown(source_address, Memory::BORKED3DS_PAGE_SIZE);
            u32 page_offset = source_address - page_start;
            u32 num_pages = Common::AlignUp(page_offset + size, Memory::BORKED3DS_PAGE_SIZE) >>
            ...
            if (reply) {
                ...
                VAddr prev_reserve = page_start - Memory::BORKED3DS_PAGE_SIZE;
                VAddr next_reserve = page_start + num_pages * Memory::BORKED3DS_PAGE_SIZE;
                ...
                Result result = src_process->vm_manager.UnmapRange(
                    page_start - Memory::BORKED3DS_PAGE_SIZE,
                    (num_pages + 2) * Memory::BORKED3DS_PAGE_SIZE);
            ...
            std::shared_ptr<BackingMem> buffer =
                std::make_shared<BufferMem>((num_pages + 2) * Memory::BORKED3DS_PAGE_SIZE);
            memory.ReadBlock(*src_process, source_address,
                             buffer->GetPtr() + Memory::BORKED3DS_PAGE_SIZE + page_offset, size);
            ...
            ASSERT(dst_process->vm_manager.ChangeMemoryState(
                       low_guard_address, Memory::BORKED3DS_PAGE_SIZE, Kernel::MemoryState::Shared,
                       Kernel::VMAPermission::ReadWrite, Kernel::MemoryState::Reserved,
                       Kernel::VMAPermission::None) == ResultSuccess);
            ASSERT(dst_process->vm_manager.ChangeMemoryState(
                       high_guard_address, Memory::BORKED3DS_PAGE_SIZE, Kernel::MemoryState::Shared,
                       Kernel::VMAPermission::ReadWrite, Kernel::MemoryState::Reserved,
                       Kernel::VMAPermission::None) == ResultSuccess);

            // Get proper mapped buffer address and store it in the cmd buffer.
            target_address += Memory::BORKED3DS_PAGE_SIZE;
