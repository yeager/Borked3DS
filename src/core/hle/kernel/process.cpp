...
void Process::ParseKernelCaps(const u32* kernel_caps, std::size_t len) {
    for (std::size_t i = 0; i < len; ++i) {
        ...
        if (descriptor == 0xFFFFFFFF) {
            ...
            AddressMapping mapping;
            mapping.address = descriptor << 12;
            mapping.size = Memory::BORKED3DS_PAGE_SIZE;
...
ResultVal<VAddr> Process::AllocateThreadLocalStorage() {
    ...
    if (needs_allocation) {
        ...
        auto offset = base_memory_region->LinearAllocate(Memory::BORKED3DS_PAGE_SIZE);
        if (!offset) {
        ...
        holding_tls_memory +=
            MemoryRegionInfo::Interval(*offset, *offset + Memory::BORKED3DS_PAGE_SIZE);
        memory_used += Memory::BORKED3DS_PAGE_SIZE;
        ...
        auto tls_page_addr =
            Memory::TLS_AREA_VADDR + static_cast<VAddr>(tls_page) * Memory::BORKED3DS_PAGE_SIZE;
        vm_manager.MapBackingMemory(tls_page_addr, kernel.memory.GetFCRAMRef(*offset),
                                    Memory::BORKED3DS_PAGE_SIZE, MemoryState::Locked);
    ...
    auto tls_address = Memory::TLS_AREA_VADDR +
                       static_cast<VAddr>(tls_page) * Memory::BORKED3DS_PAGE_SIZE +
