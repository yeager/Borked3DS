...
u32 CROHelper::Fix(u32 fix_level) {
    ...
    fix_end = Common::AlignUp(fix_end, Memory::BORKED3DS_PAGE_SIZE);
...
std::tuple<VAddr, u32> CROHelper::GetExecutablePages() const {
    ...
    for (u32 i = 0; i < segment_num; ++i) {
        ...
        if (entry.type == SegmentType::Code && entry.size != 0) {
            VAddr begin = Common::AlignDown(entry.offset, Memory::BORKED3DS_PAGE_SIZE);
            VAddr end = Common::AlignUp(entry.offset + entry.size, Memory::BORKED3DS_PAGE_SIZE);
