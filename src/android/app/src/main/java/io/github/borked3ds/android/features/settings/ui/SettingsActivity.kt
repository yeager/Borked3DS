...
class SettingsActivity : AppCompatActivity(), SettingsActivityView {
    ...
    fun onSettingsReset() {
        ...
        val editor =
            PreferenceManager.getDefaultSharedPreferences(Borked3DSApplication.appContext).edit()
        ...
        if (DirectoryInitialization.setBorked3DSUserDirectory()) {
            Borked3DSApplication.documentsTree.setRoot(Uri.parse(DirectoryInitialization.userPath))
            NativeLibrary.createConfigFile()
        } else {
            throw IllegalStateException("Borked3DS directory unavailable when accessing config file!")
        }

        // Set default values for system config file
        SystemSaveGame.apply {
            setUsername("BORKED3DS")
            setBirthday(3, 25)
            setSystemLanguage(1)
