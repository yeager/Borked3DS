 ...
 * with the native side of the Borked3DS code.
 */
object NativeLibrary {
    ...
    init {
        try {
            System.loadLibrary("borked3ds-android")
    ...
    val isPortraitMode: Boolean
        get() = Borked3DSApplication.appContext.resources.configuration.orientation ==
    ...
    /**
     * Logs the Borked3DS version, Android version and, CPU.
     */
    ...
    fun createDir(directory: String, directoryName: String): Boolean =
        if (FileUtil.isNativePath(directory)) {
            Borked3DSApplication.documentsTree.createDir(directory, directoryName)
        } else {
    ...
    fun openContentUri(path: String, openMode: String): Int =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.openContentUri(path, openMode)
        } else {
    ...
    fun getFilesName(path: String): Array<String?> =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.getFilesName(path)
        } else {
    ...
    fun getSize(path: String): Long =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.getFileSize(path)
        } else {
    ...
    fun fileExists(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.exists(path)
        } else {
    ...
    fun isDirectory(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.isDirectory(path)
        } else {
    ...
    fun copyFile(
        ...
        if (FileUtil.isNativePath(sourcePath) &&
            ...
            Borked3DSApplication.documentsTree
    ...
    fun renameFile(path: String, destinationFilename: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.renameFile(path, destinationFilename)
        } else {
    ...
    fun deleteDocument(path: String): Boolean =
        if (FileUtil.isNativePath(path)) {
            Borked3DSApplication.documentsTree.deleteDocument(path)
        } else {
