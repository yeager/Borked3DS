...
void CubebInput::StartSampling(const InputParameters& params) {
    ...
    auto init_result = cubeb_init(&impl->ctx, "Borked3DS Input", nullptr);
    ...
    auto stream_init_result = cubeb_stream_init(
        impl->ctx, &impl->stream, "Borked3DS Microphone", input_device, &input_params, nullptr,
...
std::vector<std::string> ListCubebInputDevices() {
    ...
    if (cubeb_init(&ctx, "Borked3DS Input Device Enumerator", nullptr) != CUBEB_OK) {
