...
#include "borked3ds_qt/multiplayer/lobby.h"
#include "borked3ds_qt/multiplayer/lobby_p.h"
#include "borked3ds_qt/multiplayer/message.h"
...
Lobby::Lobby(Core::System& system_, QWidget* parent, QStandardItemModel* list,
    ...
    if (ui->nickname->text().isEmpty() && !NetSettings::values.borked3ds_username.empty()) {
        // Use Borked3DS Web Service user name as nickname by default
        ui->nickname->setText(QString::fromStdString(NetSettings::values.borked3ds_username));
    }
...
void Lobby::OnJoinRoom(const QModelIndex& source) {
    ...
    QFuture<void> f = QtConcurrent::run([this, nickname, ip, port, password, verify_UID] {
        std::string token;
#ifdef ENABLE_WEB_SERVICE
        if (!NetSettings::values.borked3ds_username.empty() &&
            !NetSettings::values.borked3ds_token.empty()) {
