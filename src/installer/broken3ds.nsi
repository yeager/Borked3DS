...
!define PRODUCT_NAME "Borked3DS"
!define PRODUCT_PUBLISHER "Borked3DS Emulator Developers"
!define PRODUCT_WEB_SITE "https://borked3ds.github.io/"
...
Name "${PRODUCT_NAME}"
OutFile "borked3ds-${PRODUCT_VERSION}-windows-${PRODUCT_VARIANT}-installer.exe"
...
; MUI Settings
!define MUI_ICON "../../dist/borked3ds.ico"
...
Section "Base"
  ...
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\$DisplayName.lnk" "$INSTDIR\borked3ds.exe"
  ${If} $DesktopShortcut == 1
    CreateShortCut "$DESKTOP\$DisplayName.lnk" "$INSTDIR\borked3ds.exe"
  ${EndIf}
...
Section -Post
  ...
  WriteRegStr SHCTX "${PRODUCT_DIR_REGKEY}" "" "$INSTDIR\borked3ds.exe"
  ...
  WriteRegStr SHCTX "${PRODUCT_UNINST_KEY}" "DisplayIcon" "$INSTDIR\borked3ds.exe"
...
Section Uninstall
  ...
  Delete "$INSTDIR\*.dll"
  Delete "$INSTDIR\borked3ds.exe"
  Delete "$INSTDIR\borked3ds-cli.exe"
  Delete "$INSTDIR\borked3ds-room.exe"
